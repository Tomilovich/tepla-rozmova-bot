...
def init_db():
    with get_connection() as conn:
        ...
        -- ТАБЛИЦЯ ЗАМОВЛЕНЬ (ДЛЯ БАРИСТА ТА АНАЛІТИКИ)
...
def save_order(user_id, items, total, p_type, address):
    """Зберігає замовлення та повертає його номер (ID)"""
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO orders (user_id, items, total_price, pickup_type, address) VALUES (?, ?, ?, ?, ?)",
            (user_id, items, total, p_type, address)
        )
        return cursor.lastrowid

def update_order_status(order_id, new_status):
    """Оновлює статус замовлення (для панелі бариста)"""
    with get_connection() as conn:
        conn.execute(
...
def save_favorite(user_id: int, drink_id: str):
    user = get_user_data(user_id)

    if drink_id not in user["favorites"]:
        user["favorites"].append(drink_id)

        with get_connection() as conn:
            conn.execute(
                "UPDATE users SET favorites=? WHERE user_id=?",
                (json.dumps(user["favorites"]), user_id)
            )
